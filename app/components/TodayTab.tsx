...
export default function TodayTab({ onNavigate }: TodayTabProps) {
  ...
  const setStepsManual = () => {
    const v = parseInt(stepInput.replace(/,/g, ""), 10);
    if (!isNaN(v) && v >= 0) {
      setSteps(v);
      persist({ steps: v });
    }
  ...
  const progress = Math.round(
    ((activityDone ? 1 : 0) + (sleepHours && sleepHours >= 7 ? 1 : 0) + (steps >= STEP_GOAL ? 1 : 0)) / 3 * 100
  );
    ...
    <div className="px-4 py-4 space-y-5">
      ...
      <div className="bg-[#0f0f1a] border border-gray-800 rounded-2xl p-4">
        ...
        <div className="h-2.5 bg-gray-800 rounded-full overflow-hidden mb-3">
          <div
            className={`h-full rounded-full transition-all duration-500 ${steps >= STEP_GOAL ? "bg-green-400" : "bg-yellow-400"}`}
            style={{ width: `${stepPct}%` }}
          />
        </div>
      ...
      {activity === "bike" && <ActiveCard title="Mountain Bike" color="green"
        tip="Any length counts. The elevation and resistance burn serious calories — enjoy it." />}
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <StatCard label="Steps" value={steps >= 1000 ? `${(steps / 1000).toFixed(1)}k` : `${steps}`} color={steps >= STEP_GOAL ? "green" : "yellow"} />
        <StatCard label="Active" value={activityDone ? "Done" : "—"} color={activityDone ? "green" : "gray"} />
      </div>
